import{g as I,c as ie,d as T,f as v,h as ce,j as W,p as Y,T as pe,k as V,m as se,n as ue,o as ae,s as h,R as u,q as he,u as me,v as ge,x as l,S as fe,t as d,e as j,a as $,w as X,y as oe,r as q,L as Q,z as be,A as ye,B as ve,b as J,C as ee,i as R}from"./search-engine-c2aad9f4.js";import{I18n as a}from"./translations-a98a8f50.js";import{_}from"./index-bde76898.js";import"./gamepad-events-7f4c04b9.js";const we="https://madcampos.dev/sdrlog/assets/data-74ecd8d6.json",O={"de-DE":{icon:"🇩🇪",name:a.t`German`},"fr-FR":{icon:"🇫🇷",name:a.t`French`},"jp-JP":{icon:"🇯🇵",name:a.t`Japanese`},"es-ES":{icon:"🇪🇸",name:a.t`Spanish`},"hu-HU":{icon:"🇭🇺",name:a.t`Hungarian`},"it-IT":{icon:"🇮🇹",name:a.t`Italian`},"pt-BR":{icon:"🇧🇷",name:a.t`Brazilian Portuguese`},"cs-CZ":{icon:"🇨🇿",name:a.t`Czech`},"he-IL":{icon:"🇮🇱",name:a.t`Hebrew`},"pl-PL":{icon:"🇵🇱",name:a.t`Polish`},"fi-FI":{icon:"🇫🇮",name:a.t`Finnish`},"en-US":{icon:"🇺🇸",name:a.t`English`}},z=["Catalyst Game Labs","Cliffhanger Productions","FASA Corporation","Fantasy Productions","Harebrained Schemes","Pegasus Spiele","WizKids Games","Heyne Verlag","Other","Unofficial"],U={novel:{icon:"📚",name:a.t`Novel`},sourcebook:{icon:"📜",name:a.t`Sourcebook`},mission:{icon:"🗺️",name:a.t`Mission`},rulebook:{icon:"📝",name:a.t`Rulebook`},misc:{icon:"🔣",name:a.t`Misc.`},magazine:{icon:"📰",name:a.t`Magazine`},boardgame:{icon:"♟️",name:a.t`Boardgame`},videogame:{icon:"🎮",name:a.t`Videogame`},tcg:{icon:"🃏",name:a.t`T.C.G.`},unofficial:{icon:"📓",name:a.t`Unofficial`}},N={digital:{icon:"💽",name:a.t`Digital`},scan:{icon:"📠",name:a.t`Scan`},ocr:{icon:"💾",name:a.t`OCR`},print:{icon:"🖨️",name:a.t`Print`},physical:{icon:"🎲",name:a.t`Physical`}},B={ok:{icon:"✅",name:a.t`OK`},missing:{icon:"❌",name:a.t`Missing`},outofscope:{icon:"⛔",name:a.t`Out of scope`},canceled:{icon:"🚫",name:a.t`Canceled`}};async function re(){try{const e=await fetch(we);if(e.ok)return(await e.json()).items}catch(e){console.error("Failed to load data.",e)}return[]}async function K(){const e=await I("items"),t=await re(),s=new Map;for(const o of e)s.set(o.sku[0],o);for(const o of t)s.set(o.sku[0],o);return await ie("items",[...s.entries()]),[...s.values()]}function ne(e){function t(i){const c=(Array.isArray(i)?i:[]).filter(y=>y).map(y=>y.toString());return[...new Set(c)]}function s(i,r,n){const c=i?.toString()??"";return Object.keys(r).includes(c)?c:n}const o={name:e.name?.toString()??"",description:e.description?.toString()??"",notes:e.notes?.toString()??"",edition:Number.parseInt(e.edition?.toString()??"0"),gameDate:/^\d{4}-\d{2}$/iu.test(e.gameDate?.toString()??"")?e.gameDate?.toString():"",category:s(e.category,U,""),status:s(e.status,B,"missing"),type:s(e.type,N,""),publisher:t(e.publisher).filter(i=>z.includes(i)),sku:t(e.sku),originalLanguage:s(e.originalLanguage,O,""),releaseDate:t(e.releaseDate)};if(e.names!==null&&typeof e.names=="object"){o.names={};for(const[i,r]of Object.entries(e.names))/^[a-z]-[A-Z]/u.test(i)&&typeof r=="string"&&(o.names[i]=r)}if(e.links!==null&&typeof e.links=="object"){o.links={};for(const[i,r]of Object.entries(e.links))typeof i=="string"&&typeof r=="string"&&(o.links[i]=r)}return o}async function $e(){const e=T.createOverlay({title:a.t`Read data file`});try{const[t]=await window.showOpenFilePicker({id:"dataFile",startIn:"downloads",excludeAcceptAllOption:!1,types:[{description:a.t`JSON Files`,accept:{"text/json":[".json"]}}]});await v("files",void 0,{filePath:"/data.json",fileName:"data.json",fileExtension:".json",mimeType:"text/json",handler:t,hash:await ce(await(await t.getFile()).arrayBuffer())});const s=await t.getFile(),o=JSON.parse(await s.text());if(!o.items)throw new Error("No items found in data file.");const i=o.items.map(r=>ne(r));await ie("items",i.map(r=>[r.sku[0],r]))}catch(t){console.error("Failed to open data file.",t)}e.remove()}async function ke(e,t){const{cover:s,files:o,...i}=t;await v("items",e,i);for await(const r of o??[])await W("files","hash",r.hash)||await v("files",void 0,r);if(s)try{const r=await Y(s);await v("covers",e,r);const n=await Y(s,{referenceWidth:pe});await v("thumbs",e,n)}catch(r){console.error(`Failed to save material for id "${e}".`,r)}}const L={};function le(e={},t=navigator.language){const s=JSON.stringify({language:t,...e});return L[s]||(L[s]=new Intl.Collator(t,e)),(o,i)=>L[s].compare(o,i)}const M={};function xe(e,t=navigator.language,s={day:"2-digit",month:"short",timeZone:"UTC",year:"numeric"}){return M[t]||(M[t]=new Intl.DateTimeFormat(t,s)),M[t].format(e)}async function Fe(){const e=T.createOverlay({title:a.t`Export Covers`});try{const t=await window.showDirectoryPicker({id:"coversFolder",startIn:"downloads"}),s=await I("covers");e.total=s.length;for await(const o of s){e.increment(o.name);try{await t.getFileHandle(o.name,{create:!1})}catch{const r=await(await t.getFileHandle(o.name,{create:!0})).createWritable({keepExistingData:!1});await r.truncate(0),await r.write(o),await r.close()}}}catch(t){console.error("Failed to save covers.",t)}e.remove()}async function Ie(){const e=T.createOverlay({title:a.t`Export Thumbnails`});try{const t=await window.showDirectoryPicker({id:"thumbsFolder",startIn:"downloads"}),s=await I("thumbs");e.total=s.length;for await(const o of s){e.increment(o.name);try{await t.getFileHandle(o.name,{create:!1})}catch{const r=await(await t.getFileHandle(o.name,{create:!0})).createWritable({keepExistingData:!1});await r.truncate(0),await r.write(o),await r.close()}}}catch(t){console.error("Failed to save thmbs.",t)}e.remove()}async function Se(){const e=T.createOverlay({title:a.t`Export Data`});try{const t=await I("items");if(t.length>0){const o=await(await window.showSaveFilePicker({id:"dataFile",startIn:"downloads",suggestedName:"data.json",excludeAcceptAllOption:!0,types:[{description:a.t`JSON Files`,accept:{"text/json":[".json"]}}]})).createWritable();await o.truncate(0),await o.write(JSON.stringify({$schema:"./data.schema.json",items:t},null,"	")),await o.close()}}catch(t){console.error("Failed to export data file.",t)}e.remove()}async function Pe(e){const s=await(await window.showSaveFilePicker({id:"dataFile",startIn:"downloads",suggestedName:`${e.sku[0]}.json`,excludeAcceptAllOption:!0,types:[{description:a.t`JSON Files`,accept:{"text/json":[".json"]}}]})).createWritable();await s.truncate(0),await s.write(JSON.stringify(e,null,"	")),await s.close()}async function Oe(){const e=await K(),t=[],s=[];for await(const o of e){const[i]=o.sku,r=await V("covers",i),n=await V("thumbs",i);if(!r)try{(await fetch(`https://madcampos.dev/sdrlog/images/covers/${i}.jpg`,{method:"HEAD"})).ok||t.push(i)}catch{t.push(i)}if(!n)try{(await fetch(`https://madcampos.dev/sdrlog/images/thumbs/${i}.jpg`,{method:"HEAD"})).ok||s.push(i)}catch{s.push(i)}}return{missingCovers:t.sort(),missingThumbs:s.sort()}}async function _e(){const e=await re(),t=new Map,s=[];for(const o of e){const[i]=o.sku;if(t.get(i)){s.push(i);continue}t.set(i,o)}return s.sort()}async function Ce(){const e=await K(),t=[],s=[];for await(const o of e){const[i]=o.sku,r=await se("files","itemId",i);r.length===0&&o.status!=="canceled"&&t.push(i),r.length===0&&o.status==="ok"&&s.push(i)}return{materialsWithMissingFiles:t.sort(),materialsWithOkStatusButMissingFiles:s.sort()}}async function De(){const e=await ue("items"),t=await I("files"),s=[];for await(const o of t){if(o.handler.kind!=="file")continue;const i=o.itemId;(!i||!e.includes(i))&&s.push(o.filePath)}return s}async function Ae(){const e=await I("files"),t=e.map(i=>i.hash),s=[],o=[];for await(const[i,r]of t.entries())if(!o.includes(r)&&t.indexOf(r)!==i){const c=e.filter(p=>p.hash===r).map(p=>p.filePath);s.push(c),o.push(r)}return s}async function Ee(){const e=await I("files"),t=[],s=[];for await(const o of e){const i=o.itemId;if(!i||s.includes(i))continue;const c=e.filter(p=>p.itemId===i).map(p=>({filePath:p.filePath,...ae(p.filePath.split("/").pop()??"")})).filter(p=>!p.modifier);if(c.length>1){const p=c.map(y=>y.filePath);t.push(p),s.push(i)}}return t}async function je(){const e=await _e(),t=await Oe(),s=await Ce(),o=await De(),i=await Ae(),r=await Ee();console.log({duplicateIds:e,missingCovers:t,missingFiles:s,extraFiles:o,duplicateFiles:i,duplicateIdFiles:r})}var Te=Object.defineProperty,Re=Object.getOwnPropertyDescriptor,k=(e,t,s,o)=>{for(var i=o>1?void 0:o?Re(t,s):t,r=e.length-1,n;r>=0;r--)(n=e[r])&&(i=(o?n(t,s,i):n(i))||i);return o&&i&&Te(t,s,i),i};let m=class extends h{static shadowRootOptions={...h.shadowRootOptions,delegatesFocus:!0};#t=!1;constructor(){super(),this.cards=[],this.hidden=!1,this.hasFileSystem=!1,"showOpenFilePicker"in window&&(this.hasFileSystem=!0),this.isDevMode=!1,this.filters=[{icon:"📜",label:"$t{Sourcebooks}",filter:"category: sourcebook"},{icon:"📝",label:"$t{Rulebooks}",filter:"category: rulebook"},{icon:"🗺️",label:"$t{Adventures & Campaigns}",filter:"category: mission"},{icon:"📚",label:"$t{Novels}",filter:"category: novel"},{icon:"📰",label:"$t{Magazines}",filter:"category: magazine"},{icon:"♟️",label:"$t{Tabletop}",filter:"category: boardgame"},{icon:"🃏",label:"$t{Trading Card Game}",filter:"category: tcg"},{icon:"🎮",label:"$t{Video Games}",filter:"category: videogame"},{icon:"📓",label:"$t{Unofficial}",filter:"category: unofficial"},{icon:"🔣",label:"$t{Misc.}",filter:"category: misc"},null,{icon:"📚",label:"$t{All}",filter:"category: all"}],this.appMenuItems=[{icon:"ℹ️",label:"$t{Tool Information}",action:async()=>u.navigate("/info")},{icon:"💬",label:"$t{Language Settings}",action:async()=>u.navigate("/settings/language")},{icon:"🌓",label:"$t{Theme Settings}",action:async()=>u.navigate("/settings/theme")}],this.fileMenuItems=[null,{icon:"📥",label:"$t{Import Files}",action:he},{icon:"📦",label:"$t{Import Data}",action:$e},null,{icon:"📂",label:"$t{Import Covers}",action:me},{icon:"🧩",label:"$t{Extract Covers}",action:ge},null,{icon:"📤",label:"$t{Export Data}",action:Se},{icon:"🎴",label:"$t{Export Thumbnails}",action:Ie},{icon:"🖼️",label:"$t{Export Covers}",action:Fe}],this.devMenuItems=[null,{icon:"⛔️",label:"Report Data Inconsistencies",action:je},{icon:"💬",label:"Open CBZ reader",action:async()=>u.navigate("/cbz/test")},{icon:"🕹️",label:"Open Emulator",action:async()=>u.navigate("/emulator/test")},{icon:"📖",label:"Open Epub reader",action:async()=>u.navigate("/epub/test")}]}createRenderRoot(){return this}navigate(){this.hidden=!1}shouldUpdate(e){return this.#t&&super.shouldUpdate(e)}render(){return l`
			<sdr-menu-bar>
				<sdr-dropdown id="filters" icon="︙" trigger-button="x">
					${this.filters.map(e=>e===null?l`<sdr-dropdown-item separator></sdr-dropdown-item>`:l`
							<sdr-dropdown-item
								icon="${e.icon}"
								@click="${()=>fe.updateSearchResults(e.filter)}"
							>
								${e.label}
							</sdr-dropdown-item>
						`)}
...
`;var Ze=Object.defineProperty,He=Object.getOwnPropertyDescriptor,C=(e,t,s,o)=>{for(var i=o>1?void 0:o?He(t,s):t,r=e.length-1,n;r>=0;r--)(n=e[r])&&(i=(o?n(t,s,i):n(i))||i);return o&&i&&Ze(t,s,i),i};let w=class extends h{static shadowRootOptions={...h.shadowRootOptions,delegatesFocus:!0};static styles=q(Ke);#t;constructor(){super(),this.resetMaterial()}#e(e,t){const s=e.target;this.material[t]=t==="edition"?Number.parseInt(s.value):s.value}#s(e,t){const o=e.target.querySelector("sdr-edit-box, sdr-select");let i="";this.material[t]?.includes(o.value)?i=a.t`Item already exists in the list.`:o.value===""&&(i=a.t`Please fill the field.`),o.setCustomValidity(i),o.reportValidity()&&(this.material[t]?.push(o.value),this.requestUpdate("material"))}#i(e,t){const s=e.target,[o,i]=s.querySelectorAll("sdr-edit-box, sdr-select");this.material[t]?.[o.value]!==void 0?o.setCustomValidity(a.t`Item already exists in the list.`):o.value===""&&o.setCustomValidity(a.t`Please fill the field.`),i.value===""&&i.setCustomValidity(a.t`Please fill the field.`),o.reportValidity()&&i.reportValidity()&&(this.material[t]={...this.material[t]??{},[o.value]:i.value},this.requestUpdate("material"))}#a(e){const t=e.detail.value,s=e.target.closest("sdr-edit-list");if(!s)return;const o=this.material[s.id];Array.isArray(o)?o.splice(o.indexOf(t),1):this.material[s.id]=Object.fromEntries(Object.entries(o).filter(([i])=>i!==t)),this.requestUpdate("material")}#o(e){const t=e.detail.file;this.coverUrl=Q,this.coverUrl=URL.createObjectURL(t)}async#r(){if(this.material.sku.length>0){const[e]=await window.showOpenFilePicker({id:"newMaterialFile",startIn:"downloads",excludeAcceptAllOption:!1}),t=await be(e);this.files.push(t),this.requestUpdate("files")}}async#n(e,t){e.preventDefault(),e.stopPropagation();const s=await W("files","hash",t);s&&await Ne(s)}async#l(){const[e]=this.material.sku;if(e){if(this.isDisplaying=!0,await ke(e,{...this.material,cover:this.#t,files:this.files}),!document.querySelector(`sdr-card[id="${e}"]`)){const t=new ye({name:this.material.name,id:e,sku:this.material.sku,edition:this.material.edition,category:this.material.category,type:this.material.type,status:this.material.status});document.querySelector("main")?.append(t)}alert(`${a.t`Item #`} ${e} ${a.t`saved successfully.`}`)}}#d(){this.open=!1,u.navigate("/")}async navigate(e){let t=a.t`New Material`;return e.params.id&&(this.resetMaterial(),await this.setMaterial(e.params.id),t=this.material.name),"launchQueue"in window&&window.launchQueue.setConsumer(async s=>{this.resetMaterial();for await(const o of s.files)if(o.kind==="file")try{const r=await(await o.getFile()).text(),n=JSON.parse(r);this.material=ne(n);break}catch(i){console.error(i)}}),this.open=!0,t}resetMaterial(){this.isDisplaying=!1,this.material={category:"",type:"",sku:[],name:"",names:{},description:"",edition:0,publisher:[],gameDate:"",releaseDate:[],status:"",originalLanguage:"",notes:"",links:{}},this.files=[],this.coverUrl=Q}async setMaterial(e){const t=await V("items",e);t&&(this.isDisplaying=!0,this.material=t,ve(t.sku[0]).then(s=>{this.coverUrl=s}),se("files","itemId",t.sku[0]).then(s=>{for(const o of s)this.files.push(o)}))}firstUpdated(e){super.firstUpdated(e),this.renderRoot.querySelectorAll("sdr-edit-list").forEach(t=>{t.addEventListener("itemremoved",s=>{this.#a(s)})})}createRenderRoot(){return this}render(){return l`
						...
						<sdr-tab-panel slot="tabpanel">
							<div id="item-info">
								...
								<sdr-select id="category" required ?disabled="${this.isDisplaying}" value="${this.material.category}" @input="${e=>this.#e(e,"category")}" @change="${e=>this.#e(e,"category")}">
									...
									${P(Object.keys(U),()=>Object.entries(U).map(([e,t])=>l`
								...
								<sdr-select id="type" required ?disabled="${this.isDisplaying}" value="${this.material.type}" @input="${e=>this.#e(e,"type")}" @change="${e=>this.#e(e,"type")}">
									...
									${P(Object.keys(N),()=>Object.entries(N).map(([e,t])=>l`
										<option value="${e}">${t.icon} ${t.name}</option>
									`))}
								</sdr-select>
								...
								<sdr-select id="originalLanguage" required ?disabled="${this.isDisplaying}" value="${this.material.originalLanguage}" @input="${e=>this.#e(e,"originalLanguage")}" @change="${e=>this.#e(e,"originalLanguage")}">
									...
									${P(Object.keys(O),()=>Object.entries(O).map(([e,t])=>l`
										<option value="${e}">${t.icon} ${t.name}</option>
									`))}
								</sdr-select>
								...
								<sdr-select id="status" required ?disabled="${this.isDisplaying}" value="${this.material.status}" @input="${e=>this.#e(e,"status")}" @change="${e=>this.#e(e,"status")}">
									...
									${P(Object.keys(B),()=>Object.entries(B).map(([e,t])=>l`
										<option value="${e}">${t.icon} ${t.name}</option>
									`))}
								</sdr-select>
								...
								<sdr-edit-list id="names" ?disabled="${this.isDisplaying}" @itemadded="${e=>this.#i(e,"names")}">
									...
									${Object.entries(this.material.names??{}).map(([e,t])=>{const{name:s,icon:o}=O[e];return l`
								...
								${this.files.map(e=>l`
									<sdr-edit-list-item stretch value="${e.itemId??""}">
										<a href="#" rel="noopener noreferrer" @click="${async t=>this.#n(t,e.hash)}">
											${Je(e.fileExtension??e.mimeType??"")} ${e.fileName}${e.fileExtension}
										</a>
									</sdr-edit-list-item>
								`)}
							</sdr-edit-list>
		...
		`}};C([j({type:Boolean,reflect:!0,attribute:"disabled"})],w.prototype,"isDisplaying",2);C([d()],w.prototype,"open",2);C([d({hasChanged:(e,t)=>JSON.stringify(t)!==JSON.stringify(e)})],w.prototype,"material",2);C([d()],w.prototype,"files",2);C([d()],w.prototype,"coverUrl",2);w=C([$("sdr-view-item-details")],w);var Ge=Object.defineProperty,Ye=Object.getOwnPropertyDescriptor,H=(e,t,s,o)=>{for(var i=o>1?void 0:o?Ye(t,s):t,r=e.length-1,n;r>=0;r--)(n=e[r])&&(i=(o?n(t,s,i):n(i))||i);return o&&i&&Ge(t,s,i),i};let D=class extends h{constructor(){super(),this.open=!1,this.theme="system",J("t",()=>{this.open=!this.open}),localStorage.getItem("app-theme")&&(this.theme=localStorage.getItem("app-theme"),document.body.classList.add(`theme-${this.theme}`))}#t(e){const t=e.target;localStorage.setItem("app-theme",t.value),[...document.body.classList].filter(o=>o.startsWith("theme-")).forEach(o=>{document.body.classList.remove(o)}),document.body.classList.add(`theme-${t.value}`),this.theme=t.value}#e(){this.open=!1,u.navigate("/")}navigate(){return this.open=!0,a.t`Theme Settings`}createRenderRoot(){return this}render(){return l`
		...
		`}};H([d()],D.prototype,"open",2);H([d()],D.prototype,"theme",2);D=H([$("sdr-view-theme-settings")],D);var Xe=Object.defineProperty,Qe=Object.getOwnPropertyDescriptor,G=(e,t,s,o)=>{for(var i=o>1?void 0:o?Qe(t,s):t,r=e.length-1,n;r>=0;r--)(n=e[r])&&(i=(o?n(t,s,i):n(i))||i);return o&&i&&Xe(t,s,i),i};let A=class extends h{constructor(){super(),this.open=!1,this.language="en-US",J("l",()=>{this.open=!this.open})}async#t(e){const t=e.target;t.value!==a.getLanguage()&&(t.disabled=!0,await a.setLanguage(t.value),location.reload())}#e(){this.open=!1,u.navigate("/")}navigate(){return this.open=!0,(this.shadowRoot?.querySelector("#language-select")).focus(),a.t`Language Settings`}createRenderRoot(){return this}render(){return l`
		...
		`}connectedCallback(){super.connectedCallback(),requestAnimationFrame(()=>{this.language=a.getLanguage()})}};G([d()],A.prototype,"open",2);G([d()],A.prototype,"language",2);A=G([$("sdr-view-language-settings")],A);const et=`sdr-view-app-info summary h3{display:inline;margin-inline-start:var(--margin-inline)}sdr-view-app-info dt{font-weight:700}sdr-view-app-info code{font-family:ui-monospace,monospace}
`;var tt=Object.defineProperty,it=Object.getOwnPropertyDescriptor,de=(e,t,s,o)=>{for(var i=o>1?void 0:o?it(t,s):t,r=e.length-1,n;r>=0;r--)(n=e[r])&&(i=(o?n(t,s,i):n(i))||i);return o&&i&&tt(t,s,i),i};let E=class extends h{static styles=q(et);constructor(){super(),this.open=!1,J("i",()=>{this.open=!this.open})}#t(){this.open=!1,u.navigate("/")}navigate(){return this.open=!0,a.t`Information`}createRenderRoot(){return this}render(){return l`
...
`;var rt=Object.defineProperty,nt=Object.getOwnPropertyDescriptor,S=(e,t,s,o)=>{for(var i=o>1?void 0:o?nt(t,s):t,r=e.length-1,n;r>=0;r--)(n=e[r])&&(i=(o?n(t,s,i):n(i))||i);return o&&i&&rt(t,s,i),i};const lt={select:{key:"Enter",code:"Enter",keyCode:13},start:{key:" ",code:"Space",keyCode:32},leftBumber:{key:"E",code:"KeyE",keyCode:69},rightBumber:{key:"P",code:"KeyP",keyCode:32},up:{key:"ArrowUp",code:"ArrowUp",keyCode:38},down:{key:"ArrowDown",code:"ArrowDown",keyCode:40},left:{key:"ArrowLeft",code:"ArrowLeft",keyCode:37},right:{key:"ArrowRight",code:"ArrowRight",keyCode:39},a:{key:"H",code:"KeyH",keyCode:72},b:{key:"G",code:"KeyG",keyCode:71},x:{key:"Y",code:"KeyY",keyCode:89},y:{key:"T",code:"KeyT",keyCode:84}};let g=class extends h{static shadowRootOptions={...h.shadowRootOptions,delegatesFocus:!0};static styles=q(ot);set paused(e){e?this.#e?.pauseMainLoop():this.#e?.resumeMainLoop(),this.#t=e}get paused(){return this.#t}#t=!1;#e=null;constructor(){super(),this.open=!1,this.#u(),document.addEventListener("keydown",e=>{const t=["8","9","13","19","27","32","33","34","35","36","42","44","45","91","92","93","112","113","114","115","116","117","118","119","120","121","122","123","124","125","126","127","128","129","130","131","132","133","134","135"];this.loaded&&t.includes(e.code)&&e.preventDefault()},{capture:!1,passive:!0}),document.addEventListener("keydown",e=>{this.loaded&&e.key==="F2"&&this.#p()},{capture:!1,passive:!0}),document.addEventListener("keydown",e=>{this.loaded&&e.key==="F3"&&this.#c()},{capture:!1,passive:!0}),document.addEventListener("keydown",e=>{this.loaded&&e.key==="F4"&&this.#e?._cmd_toggle_menu()},{capture:!1,passive:!0}),document.addEventListener("keydown",e=>{this.loaded&&e.key==="Escape"&&(this.paused=!this.paused)}),document.addEventListener("visibilitychange",()=>{const e=document.visibilityState==="visible",t=document.visibilityState==="hidden";this.loaded&&t&&!this.paused&&(this.paused=!0),this.loaded&&e&&this.paused&&(this.paused=!1)},!1)}#s(){this.open=!1,u.navigate("/")}#i(e,t){this.canvas.dispatchEvent(new KeyboardEvent(e,{bubbles:!0,cancelable:!1,shiftKey:!1,ctrlKey:!1,metaKey:!1,altKey:!1,...lt[t]}))}async#a(){const e=(await _(()=>import("./index-9baabb87.js"),[])).default,t=this.dpad,{width:s,height:o}=t.getBoundingClientRect(),i=e.create({zone:t,color:"white",multitouch:!1,position:{top:`${o/2}px`,left:`${s/2}px`},mode:"static",restJoystick:!0,shape:"circle",follow:!1,dynamicPage:!0});let r=null;i.on("move",(n,{direction:c})=>{r=c?.angle??null,r&&this.#i("keydown",r)}),i.on("end",()=>{r&&this.#i("keyup",r)})}#o(){const{width:e,height:t}=this.gameWrapper.getBoundingClientRect();this.#e?.setCanvasSize(e,t)}#r(e){this.#e?.FS.createPath("/",e,!0,!0)}async#n(){const e="/home/web_user/retroarch/",t=await at();this.#r(e);for await(const[s,o]of t){const i=await o.arrayBuffer();o.type==="application/x+directory"?this.#r(`${e}${o.name}`):this.#e?.FS.writeFile(`${e}${s}`,new Uint8Array(i))}}async#l(e){const t=new Map([["GENESIS","genesis_plus_gx"],["SEGA-CD","genesis_plus_gx"],["SNES","snes9x"]]),s=await Z(e),{id:o}=ae(s.name),i=t.get(o??"")??"",n=(await _(()=>import(`https://madcampos.dev/sdrlog/lib/webretro/${i}_libretro.js`),[])).default;this.#e=n({canvas:this.canvas,onRuntimeInitialized:async()=>{this.#o(),await this.#n();const c=await s.arrayBuffer();this.#e?.FS.writeFile("/rom.bin",new Uint8Array(c)),this.#e?.callMain(this.#e.arguments),this.#o(),this.loaded=!0}})}async#d(){document.fullscreenElement?await document.exitFullscreen():await this.requestFullscreen({navigationUI:"hide"})}#c(){this.#e?._cmd_load_state()}#p(){this.#e?._cmd_savefiles(),this.#e?._cmd_save_state(),window.setTimeout(async()=>{const t="/home/web_user/retroarch/userdata/saves/rom.srm",s="/home/web_user/retroarch/userdata/states/rom.state",o=this.#e?.FS.stat(t)??{size:0},i=this.#e?.FS.stat(s)??{size:0};if(o.size>0&&i.size>0){this.#e?.pauseMainLoop();const r=this.#e?.FS.readFile(t)??new Uint8Array,n=new File([r],t),c=this.#e?.FS.readFile(s)??new Uint8Array,p=new File([c],s);await v("emulator",t,n),await v("emulator",s,p),this.#e?.resumeMainLoop()}},1e3)}#u(){this.#e=null,this.loaded=!1}async navigate(e){if(this.#u(),!!e.params.id)return await this.#l(e.params.id),this.open=!0,"Emulator"}firstUpdated(e){super.firstUpdated(e),this.#a()}createRenderRoot(){return this}render(){return l`
		...
		`}};S([j({type:Boolean,reflect:!0})],g.prototype,"loaded",2);S([R("#game-canvas")],g.prototype,"canvas",2);S([R("#game-wrapper")],g.prototype,"gameWrapper",2);S([R("#dpad")],g.prototype,"dpad",2);S([d()],g.prototype,"open",2);S([d()],g.prototype,"paused",1);g=S([$("sdr-view-emulator")],g);const dt="data:text/css;base64,";var ct=Object.defineProperty,pt=Object.getOwnPropertyDescriptor,x=(e,t,s,o)=>{for(var i=o>1?void 0:o?pt(t,s):t,r=e.length-1,n;r>=0;r--)(n=e[r])&&(i=(o?n(t,s,i):n(i))||i);return o&&i&&ct(t,s,i),i};let f=class extends h{#t;constructor(){super(),this.open=!1,this.#a()}#e(){this.open=!1,u.navigate("/")}#s(e){this.loaded&&(e.key==="ArrowLeft"&&this.showPreviousPage(),e.key==="ArrowRight"&&this.showNextPage())}async#i(e){const t=await Z(e);"JSZip"in window||await _(()=>import("./jszip.min-2f4a7b1a.js").then(i=>i.j),["assets/jszip.min-2f4a7b1a.js","assets/_commonjs-dynamic-modules-3f1b5830.js"]),"ePub"in window||await _(()=>import("./epub.min-ae445f80.js").then(i=>i.e),["assets/epub.min-ae445f80.js","assets/_commonjs-dynamic-modules-3f1b5830.js","assets/jszip.min-2f4a7b1a.js"]);const s=ePub(await t.arrayBuffer()),{toc:o}=await s.loaded.navigation;this.#t=s.renderTo(this.renderArea,{width:"100%",height:"100%",flow:"scrolled-doc"}),this.#t.themes.register("dark",dt),this.#t.themes.select("dark"),this.toc=o,this.#t.on("keyup",i=>this.#s(i)),document.addEventListener("keyup",i=>this.#s(i)),this.#t.on("rendered",i=>{this.selectedPage=i.href}),this.#t.on("relocated",i=>{i.atEnd?this.nextPageVisibility="hidden":this.nextPageVisibility="visible",i.atStart?this.previousPageVisibility="hidden":this.previousPageVisibility="visible"}),await this.#t.display(),this.loaded=!0}#a(){this.loaded=!1,this.toc=[],this.selectedPage="",this.nextPageVisibility="hidden",this.previousPageVisibility="hidden",this.#t?.destroy(),this.#t=void 0}async showNextPage(){await this.#t?.next()}async showPreviousPage(){await this.#t?.prev()}async navigate(e){if(this.#a(),this.renderArea.innerHTML="",!!e.params.id)return await this.#i(e.params.id),this.open=!0,"Epub Reader"}createRenderRoot(){return this}render(){return l`
		...
		`}};x([j({type:Boolean,attribute:"loaded"})],f.prototype,"loaded",2);x([d()],f.prototype,"open",2);x([d()],f.prototype,"toc",2);x([d()],f.prototype,"selectedPage",2);x([d()],f.prototype,"nextPageVisibility",2);x([d()],f.prototype,"previousPageVisibility",2);x([R("#book")],f.prototype,"renderArea",2);f=x([$("sdr-view-epub-reader")],f);var ut=Object.defineProperty,ht=Object.getOwnPropertyDescriptor,F=(e,t,s,o)=>{for(var i=o>1?void 0:o?ht(t,s):t,r=e.length-1,n;r>=0;r--)(n=e[r])&&(i=(o?n(t,s,i):n(i))||i);return o&&i&&ut(t,s,i),i};const mt=le({ignorePunctuation:!0,numeric:!0}),te=a.t`Default section`,gt=new Map([[".png","image/png"],[".apng","image/apng"],[".jpg","image/jpeg"],[".jpeg","image/jpeg"],[".jfif","image/jpeg"],[".pjpeg","image/jpeg"],[".pjp","image/jpeg"],[".avif","image/avif"],[".webp","image/webp"],[".bmp","image/bmp"],[".gif","image/gif"]]);let b=class extends h{static shadowRootOptions={...h.shadowRootOptions,delegatesFocus:!0};#t;constructor(){super(),this.open=!1,this.#o(),document.addEventListener("keyup",e=>{this.loaded&&(e.key==="ArrowLeft"&&this.showPreviousPage(),e.key==="ArrowRight"&&this.showNextPage())},!1),window.addEventListener("wheel",e=>{this.loaded&&(e.shiftKey||(e.preventDefault(),this.renderRoot.querySelector("article")?.scrollBy({left:e.deltaY,behavior:"smooth"})))},{capture:!1,passive:!1})}#e(){this.open=!1,u.navigate("/")}#s([e]){this.#t=e.target,this.#t.previousElementSibling?this.previousPageVisibility="visible":this.previousPageVisibility="hidden",this.#t.nextElementSibling?this.nextPageVisibility="visible":this.nextPageVisibility="hidden",this.selectedPage=this.#t.dataset.folder}async#i(e){if(!e)return{};"JSZip"in window||await _(()=>import("./jszip.min-2f4a7b1a.js").then(i=>i.j),["assets/jszip.min-2f4a7b1a.js","assets/_commonjs-dynamic-modules-3f1b5830.js"]);const t=await JSZip.loadAsync(e),s={};for await(const i of Object.values(t.files))if(!i.dir){const r=await i.async("blob"),[n,c=te]=i.name.split("/").reverse(),p=/(?<extension>\.[a-z0-9]{3,})$/u,{extension:y}=p.exec(n)?.groups??{};gt.has(y)&&(c in s||(s[c]=[]),s[c].push({name:n,folder:c,url:URL.createObjectURL(r)}))}return Object.fromEntries(Object.keys(s).sort((i,r)=>r===te?1:mt(i,r)).map(i=>[i,s[i]]))}async#a(e){const t=await Z(e),s=await this.#i(t);for(const i of Object.keys(s)){this.toc.push(i);for(const r of s[i])this.pages.push(r)}const o=new IntersectionObserver(i=>this.#s(i),{threshold:1});this.renderRoot.querySelectorAll("img").forEach(i=>o.observe(i)),this.loaded=!0,[this.selectedPage]=this.toc,this.renderRoot.querySelector("article img:first-child")?.scrollIntoView()}#o(){this.loaded=!1,this.selectedPage="",this.toc=[],this.pages=[],this.nextPageVisibility="hidden",this.previousPageVisibility="hidden"}showNextPage(){this.#t?.nextElementSibling?.scrollIntoView()}showPreviousPage(){this.#t?.previousElementSibling?.scrollIntoView()}async navigate(e){if(this.#o(),!!e.params.id)return await this.#a(e.params.id),this.open=!0,"Comic Book Reader"}createRenderRoot(){return this}render(){return l`
			<sdr-dialog ?open="${this.open}" @close="${()=>this.#e()}">
				<sdr-button
					icon-button
					slot="title"
		...
		`}};F([j({type:Boolean,reflect:!0})],b.prototype,"loaded",2);F([d()],b.prototype,"open",2);F([d()],b.prototype,"selectedPage",2);F([d()],b.prototype,"pages",2);F([d()],b.prototype,"toc",2);F([d()],b.prototype,"nextPageVisibility",2);F([d()],b.prototype,"previousPageVisibility",2);b=F([$("sdr-view-cbz-reader")],b);u.init({baseUrl:"https://madcampos.dev/sdrlog/",routes:[{path:"/",view:m},{path:"/item/:id?",view:w},{path:"/info",view:E},{path:"/settings/theme",view:D},{path:"/settings/language",view:A},{path:"/emulator/:id",view:g},{path:"/epub/:id",view:f},{path:"/cbz/:id",view:b}]});
