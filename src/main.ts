...
import SdrCardComponent from './components/SdrCard.ce.vue';
import App from './App.vue';

defineCustomElement(SdrCardComponent);
...
createApp(App).mount(document.querySelector('main') as HTMLElement);
...
window.addEventListener('DOMContentLoaded', async () => {
	const { I18n } = await import('./js/intl/translations');
	...
	await import('./views');
	await import('./js/gamepad/gamepad-navigation');
	...
	const { fetchItems } = await import('./js/data/data-import');
	const { createComparer } = await import('./js/intl/formatting');
	...
	for (const material of sortedMaterials) {
		const itemCard = document.createElement(SdrCard.elementName);
